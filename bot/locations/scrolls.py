# locations/scrolls.py - с возвратом в лагерь

import sqlite3
from config import DB_NAME
from core import get_character_async, send_message, update_user_async, recalc_stats_async, get_user_async
...
async def show_scrolls(vk, user_id):
    ...
    if not char:
        await send_message(vk, user_id, 'Сначала создайте персонажа.', get_back_keyboard('инвентарь'))
        return
    ...
    if not scrolls:
        ...
        await send_message(vk, user_id, '📜 У вас нет свитков.\n\nКупить свитки можно в 💎 Премиум магазине.', keyboard)
        return
    ...
    if debuff > 0:
        debuff_names = {1: '☠️ Проклятие (-30% стат)', 2: '🔥 Печать башни (-50% стат)'}
    ...
    await update_user_async(user_id, state='scrolls', context=context)
...
async def use_curse_remove_scroll(vk, user_id, scroll_id):
    ...
    if not row or row[0] <= 0:
        conn.close()
        await send_message(vk, user_id, '❌ У вас нет этого свитка.', get_back_keyboard('лагерь'))
        await show_scrolls(vk, user_id)
        return
    ...
    await send_message(vk, user_id, f'✅ {debuff_name} снято!\nСтаты восстановлены.', get_back_keyboard('лагерь'))
    await show_scrolls(vk, user_id)
...
async def use_scroll(vk, user_id, scroll_id, scroll_type):
    if scroll_type == 'curse_remove':
        ...
        await send_message(vk, user_id, f'❌ Неизвестный тип свитка: {scroll_type}', get_back_keyboard('лагерь'))
        await show_scrolls(vk, user_id)