# locations/guild.py (ПОЛНОСТЬЮ ИСПРАВЛЕННЫЙ)
...
from guild import (
    ...
    get_guild_members_with_activity,
    disband_guild
)
...
async def show_guild(vk, user_id, parent='город'):
...
async def show_guild_members(vk, user_id):
    """Показ участников гильдии с еженедельной активностью"""
    char = await get_character_async(user_id)
    if not char:
        await send_message(vk, user_id, 'Сначала создайте персонажа.', get_back_keyboard('город'))
        return
    ...
    user_data = await get_user_async(user_id)
    context = user_data['context']
    parent = context.get('parent_state', 'город')
    context['parent_state'] = parent
    await update_user_async(user_id, context=context)
    ...
    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    cur.execute('SELECT exp, weekly_exp FROM guilds WHERE id = ?', (guild['id'],))
    guild_row = cur.fetchone()
    conn.close()
    guild_exp = guild_row[0] if guild_row else 0
    guild_weekly = guild_row[1] if guild_row else 0

    # ✅ СОЗДАЁМ ПЕРЕМЕННУЮ text
    lines = [f"👥 Участники гильдии «{guild['name']}»:\n"]
    lines.append(f"📊 Опыт гильдии: {guild_exp} / {guild_exp_to_next_level(guild['level'])}")
    lines.append(f"📈 Еженедельный опыт гильдии: {guild_weekly}\n")
    ...
    for i, m in enumerate(members, 1):
        ...
        total_exp = m.get('guild_exp', 0)
        weekly_exp = m.get('guild_exp_weekly', 0)
        
        # ✅ Если оба показателя = 0 — новая гильдия
        if total_exp == 0 and weekly_exp == 0:
            total_display = "🆕 новая гильдия"
            weekly_display = "0"
        else:
            total_display = f"📊{total_exp}"
            weekly_display = f"📈{weekly_exp}"
        
        line = f"{i}. 🆔{m['id']} {rank_icon} {m['name']} | {m['level']}ур. | {total_display} | {weekly_display} | {m['last_activity']}"
        lines.append(line)

    text = "\n".join(lines)  # ✅ СОЗДАЁМ text ИЗ lines
...
async def show_guild_storage_add(vk, user_id):
    ...
    keyboard = VkKeyboard()
    if inv_items:
        keyboard.add_button('🗡️ Предметы', color=VkKeyboardColor.PRIMARY, payload={'cmd': 'guild_storage_add_items'})
    if crystals:
        keyboard.add_button('💎 Кристаллы', color=VkKeyboardColor.PRIMARY, payload={'cmd': 'guild_storage_add_crystals'})
...
async def show_guild_manage(vk, user_id):
    ...
    if my_rank not in ('Лидер', 'Заместитель'):
        await send_message(vk, user_id, 'У вас нет прав на управление гильдией.', get_back_keyboard('город2'))
        return
    ...
    if my_rank == 'Лидер':
        keyboard.add_button('💥 Расформировать гильдию', color=VkKeyboardColor.NEGATIVE, payload={'cmd': 'guild_disband'})
        keyboard.add_line()
...
async def show_guild_list(vk, user_id, page=1):
    ...
    if not guilds:
        # ✅ КНОПКА "В ОЗЕРНЫЙ КРАЙ" (не в Стальной Трон)
        keyboard = VkKeyboard()
        keyboard.add_button('🏙️ В Озерный Край', color=VkKeyboardColor.SECONDARY, payload={'cmd': 'go_city2'})
        await send_message(vk, user_id, '📭 Пока нет гильдий. Создайте свою!', keyboard)
        return
    ...
    for i, g in enumerate(guilds, start=1 + (page - 1) * 5):
        ...
        text += f"{i}. 🆔{g['id']} {icon} {g['name']} (Ур.{g['level']}) | Участников: {g['members']}/{g['max_members']} | 💰 {g['silver']}\n"
    ...
    # ✅ КНОПКА "В ОЗЕРНЫЙ КРАЙ" (не в Стальной Трон)
    keyboard.add_button('🏙️ В Озерный Край', color=VkKeyboardColor.SECONDARY,
                       payload={'cmd': 'go_city2'})

    await send_message(vk, user_id, text, keyboard)


async def show_guild_apply_prompt(vk, user_id):
    guilds, _ = await asyncio.to_thread(get_guilds_list, 1, 10)
    
    hint = ""
    if guilds:
        hint = "\n\n📌 Доступные ID гильдий:\n"
        for g in guilds[:5]:
            hint += f"  🆔 {g['id']} — {g['name']} (Ур.{g['level']})\n"
        if len(guilds) > 5:
            hint += f"  ... и ещё {len(guilds) - 5} гильдий\n"
    
    keyboard = VkKeyboard()
    keyboard.add_button('🏙️ В Озерный Край', color=VkKeyboardColor.SECONDARY, payload={'cmd': 'go_city2'})
    
    await send_message(vk, user_id,
        f'📝 Введите ID гильдии, в которую хотите подать заявку:\n'
        f'(можно посмотреть в списке гильдий){hint}',
        keyboard)
...
async def show_guild_apply_confirm(vk, user_id, guild_id):
    ...
    if not guild:
        keyboard = VkKeyboard()
        keyboard.add_button('🏙️ В Озерный Край', color=VkKeyboardColor.SECONDARY, payload={'cmd': 'go_city2'})
        await send_message(vk, user_id, f'❌ Гильдия с ID {guild_id} не найдена.', keyboard)
        return
    ...
    if existing:
        keyboard = VkKeyboard()
        keyboard.add_button('🏙️ В Озерный Край', color=VkKeyboardColor.SECONDARY, payload={'cmd': 'go_city2'})
        await send_message(vk, user_id, f'❌ Вы уже состоите в гильдии «{existing["name"]}».', keyboard)
        return
...
async def show_guild_applications(vk, user_id):
    ...
    guild = await asyncio.to_thread(get_guild_by_character, char['id'])
    if not guild:
        await send_message(vk, user_id, 'Вы не состоите в гильдии.', get_back_keyboard('город'))
        return

    rank = await asyncio.to_thread(get_guild_rank, char['id'], guild['id'])
    if rank not in ('Лидер', 'Заместитель', 'Офицер'):
        await send_message(vk, user_id, '❌ У вас нет прав на просмотр заявок.', get_back_keyboard('город'))
        return
    ...
    if not apps:
        keyboard = VkKeyboard()
        keyboard.add_button('🏰 В гильдию', color=VkKeyboardColor.SECONDARY, payload={'cmd': 'go_guild'})
        await send_message(vk, user_id, '📭 Нет новых заявок.', keyboard)
        return
    ...
    for i, app in enumerate(apps, 1):
        ...
        text += f"{i}. 🧑 {app['name']} (Ур.{app['level']}, {app['class']}) | ID заявки: {app['id']} | Подана: {time_str}\n"

    text += "\n━━━━━━━━━━━━━━━━━━━━━━━\n"
    text += "📝 Введите ID заявки для управления."

    keyboard = VkKeyboard()
    keyboard.add_button('✅ Принять по ID', color=VkKeyboardColor.POSITIVE,
                       payload={'cmd': 'guild_accept_prompt'})
    keyboard.add_button('❌ Отклонить по ID', color=VkKeyboardColor.NEGATIVE,
                       payload={'cmd': 'guild_reject_prompt'})
    keyboard.add_line()
    keyboard.add_button('🏰 В гильдию', color=VkKeyboardColor.SECONDARY, payload={'cmd': 'go_guild'})
...
async def show_guild_accept_prompt(vk, user_id):
...
async def show_guild_reject_prompt(vk, user_id):
...
async def show_guild_quests(vk, user_id):
    char = await get_character_async(user_id)
    if not char:
        await send_message(vk, user_id, 'Сначала создайте персонажа.', get_back_keyboard('город'))
        return
    ...
    from guild_quests import get_available_guild_quests

    try:
        available = await asyncio.to_thread(get_available_guild_quests, char['id'])
    except Exception as e:
        await send_message(vk, user_id, f'❌ Ошибка при получении квестов: {e}')
        return

    if not available:
        keyboard = VkKeyboard()
        keyboard.add_button('🏰 В гильдию', color=VkKeyboardColor.SECONDARY, payload={'cmd': 'go_guild'})
        await send_message(vk, user_id, '📭 Нет доступных гильдейских квестов.', keyboard)
        return
...
async def show_guild_quest_status(vk, user_id):
    """Статус активного квеста гильдии"""
    char = await get_character_async(user_id)
    if not char:
        await send_message(vk, user_id, 'Сначала создайте персонажа.', get_back_keyboard('город'))
        return
    ...
    from guild_quests import get_active_guild_quest

    quest = await get_active_guild_quest(user_id)
    if not quest:
        keyboard = VkKeyboard()
        keyboard.add_button('🏰 В гильдию', color=VkKeyboardColor.SECONDARY, payload={'cmd': 'go_guild'})
        await send_message(vk, user_id, 'У вас нет активного квеста.', keyboard)
        return
...
async def show_guild_disband_confirm(vk, user_id):
    """Подтверждение расформирования гильдии"""
    char = await get_character_async(user_id)
    if not char:
        await send_message(vk, user_id, 'Сначала создайте персонажа.', get_back_keyboard('город'))
        return
    ...
    if guild['leader_id'] != char['id']:
        await send_message(vk, user_id, '❌ Только лидер может расформировать гильдию.', get_back_keyboard('город2'))
        return

    keyboard = VkKeyboard()
    keyboard.add_button('✅ Да, расформировать', color=VkKeyboardColor.NEGATIVE, payload={'cmd': 'guild_disband_confirm'})
    keyboard.add_button('❌ Отмена', color=VkKeyboardColor.SECONDARY, payload={'cmd': 'guild_manage'})

    await send_message(vk, user_id,
        f"⚠️ ВЫ УВЕРЕНЫ?\n\n"
        f"Вы собираетесь расформировать гильдию «{guild['name']}»!\n"
        f"Все участники будут исключены, склад очищен.\n"
        f"Это действие НЕОБРАТИМО!\n\n"
        f"Нажмите «Да, расформировать» для подтверждения.",
        keyboard)


async def show_guild_disband_execute(vk, user_id):
    """Выполнение расформирования гильдии"""
    char = await get_character_async(user_id)
    if not char:
        await send_message(vk, user_id, 'Сначала создайте персонажа.', get_back_keyboard('город'))
        return
    ...
    success, msg = await asyncio.to_thread(disband_guild, guild['id'])
    ...
    if success:
        ...
        for m in members:
            if m['id'] != char['id']:
                member_char = await get_character_by_id_async(m['id'])
                if member_char:
                    try:
                        await send_message(vk, member_char['vk_id'], 
                            f"⚠️ Гильдия «{guild_name}» была расформирована лидером.")
                    except:
                        pass
